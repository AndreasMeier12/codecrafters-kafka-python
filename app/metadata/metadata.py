...
class ClusterMetaDataLog:
    ...
    def of_bytes(cls, stuff: bytes):
        ...
        batch_length = int.from_bytes(in_file.read(4))
        partition_leader_epoch = int.from_bytes(in_file.read(4))
        magic_byte = int.from_bytes(in_file.read(1))
        crc = int.from_bytes(in_file.read(4))
        record_batch = int.from_bytes(in_file.read(2))
        ...
        last_offset_data = int.from_bytes(in_file.read(4))

        timestamp_raw = int.from_bytes(in_file.read(8))
        base_timestamp: datetime.datetime.fromtimestamp(timestamp_raw)
        timestamp_raw = int.from_bytes(in_file.read(8))
        max_timestamp: datetime.datetime.fromtimestamp(timestamp_raw)
        producer_id = int.from_bytes(in_file.read(8))
        producer_epoch = int.from_bytes(in_file.read(4))
        base_sequence = int.from_bytes(in_file.read(4))
        records_length: int.from_bytes(in_file.read(4))
